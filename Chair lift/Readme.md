...
### **Energy Monitoring Circuit Diagram**

High-side shunt on the 24V chair lift bus. The buck converter taps the same battery rail to power the ESP32-C6; INA238 shares I²C with the MCU.

![Energy monitoring wiring diagram](../chair-lift/energy-monitoring-circuit.svg)

| Connection | From | To | Notes |
|------------|------|-----|-------|
| Power rail | 24V Battery + | INA238 VIN+ | Through 0.015Ω shunt |
| Load | INA238 VIN− | Chair lift motor/driver | High-side sense |
| I²C | INA238 SDA | ESP32 GPIO19 | Address 0x40 |
| I²C | INA238 SCL | ESP32 GPIO20 | 3.3V logic |
| MCU supply | Buck OUT+ (5V) | ESP32-C6 | 24V→5V, 3A buck |
| Ground | Battery − | Common GND | ESP32, INA238, load |

> **Portfolio:** Full write-up with gallery and live specs → [project-6.html](../project-6.html) on [julfikerhaider2001.github.io/portfolio.alif](https://julfikerhaider2001.github.io/portfolio.alif/project-6.html)
...
5. **Hardware connections** (see [circuit diagram](../chair-lift/energy-monitoring-circuit.svg)):
   ```
   INA238 → ESP32-C6
   SDA → GPIO19
   SCL → GPIO20
...
![Wiring overview](../chair-lift/energy-monitoring-circuit.svg)
